...
SET FOREIGN_KEY_CHECKS=0;  -- Пришлось отключить проверку т.к. в таблицы posts есть user_id с несуществующем в таблице users id-шником, третья запись с низу с user_id = 7
ALTER TABLE posts
...
SELECT title, full_text, user_id, login FROM users, posts
WHERE users.id = user_id;

-- №3.	Выполните поиск  по публикациям, автором котоырх является клиент "Mikle".
SELECT title, full_text, user_id, login FROM users, posts
WHERE users.id = user_id AND login = "Mikle";
